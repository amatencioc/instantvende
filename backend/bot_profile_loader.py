"""Cargador y caché del perfil dinámico del bot."""
from __future__ import annotations

import json
import logging
import threading
from pathlib import Path
from typing import Any

from config import settings

_DEFAULT_PROFILE_PATH = Path(__file__).parent / "bot_profile.json"
_lock = threading.RLock()
_cached_profile: dict | None = None
_loader_logger = logging.getLogger(__name__)


def load_default_profile() -> dict:
    """Carga el perfil por defecto desde el archivo JSON."""
    path_str = getattr(settings, "BOT_PROFILE_PATH", "")
    path = Path(path_str) if path_str else _DEFAULT_PROFILE_PATH
    with open(path, encoding="utf-8") as f:
        data = json.load(f)
    data.pop("_comment", None)
    return data


def get_profile(db=None) -> dict:
    """
    Devuelve el perfil activo.
    1. Intenta leer desde la base de datos (si db está disponible).
    2. Fallback: usa el perfil por defecto del archivo JSON.
    Cachea el resultado en memoria para no hacer queries en cada mensaje.
    """
    global _cached_profile
    with _lock:
        if db is not None:
            try:
                from database import BotProfile
                row = db.query(BotProfile).order_by(BotProfile.id.desc()).first()
                if row:
                    _cached_profile = json.loads(row.profile_json)
                    return _cached_profile
            except Exception as exc:
                _loader_logger.warning("No se pudo cargar el perfil desde la DB, usando fallback", exc_info=exc)
        if _cached_profile is None:
            _cached_profile = load_default_profile()
        return _cached_profile


def invalidate_cache() -> None:
    """Invalida el caché en memoria para forzar recarga desde DB."""
    global _cached_profile
    with _lock:
        _cached_profile = None


def get_field(profile: dict, *keys: str, default: Any = None) -> Any:
    """Navega el perfil con una ruta de claves. Ej: get_field(p, 'bot', 'name')."""
    val = profile
    for key in keys:
        if not isinstance(val, dict):
            return default
        val = val.get(key, default)
        if val is default:
            return default
    return val
